#!/bin/bash
#
# ASSESSMENT_KAIGO.TOUROKU : アセスメント登録
#
# Usage : ASSESSMENT_KAIGO.TOUROKU
#
...
# データ作成
# 基本情報
# 1:利用者ID 2:作成年月日(西暦) 3:作成年月日(和暦) 4:実施場所 5:記入者 6:アセスメント理由 7:入居年月日 8:ふりがな 9:性別 10:生年月日(和暦) 11:生年月日(西暦) 12:年齢 13:利用者氏名 14:住所 15:電話番号 16:世帯区分
# 17:初回相談受付月日 18:受付方法 19:依頼者 20:相談経路 21:相談内容 22:生活暦 23:現在の生活状況 24:障害高齢者の日常生活自立度 25:認知症高齢者の日常生活自立度 26:認定介護度
# 27:認定年月日(和暦) 28:認定年月日(西暦) 29:認定期間開始日(和暦) 30:認定期間終了日(和暦)  31:認定期間開始日(西暦) 32:認定期間終了日(西暦) 33:介護保険被保険者番号 34:医療保険
# 35:身体障害者手帳有無 36:身体障害者手帳種 37:身体障害者手帳級 38:生活保護有無 39:生活保護受給者番号 40:生活保護負担者番号
echo "$Riyoushaid" "$hdShokaiSakuseidate" "$txtShokaiSakuseidate" "$slShokaiJissibasho" "$slShokaiKinyusha" "$slAssessmentRiyu" "$txtNyukyoNengappi" "$txtNyukyoshaFurigana" "$slNyukyoshaSeibetsu" "$txtNyukyoshaSeinengappi" \
...
"$txtNinteiKikanEnd" "$hdNinteiKikanStart" "$hdNinteiKikanEnd" "$txtKaigohokenHihokenshaBangou" "$txtIryouHoken" "$slShintaiShougaishaTechou" "$txtShintaiShougaishaSyu" "$txtShintaiShougaishaKyu" "$slSeikatsuhogo" \
...
"$rdADLChakui2" "$txtADLChakui2" "$rdADLChakui3" "$rdADLMidashinami1" "$txtADLMidashinami1" "$rdADLMidashinami2" "$txtADLMidashinami2" "$rdADLMidashinami3" "$txtADLMidashinami3" "$rdADLNyuyoku1" "$txtADLNyuyoku1" \
...
"$slShokujiKeitaiNaiyou3" "$txtShokujiKeitaiType3" "$rdEijouJoutai1" "$txtEiyouJoutaibasho1" "$rdEijouJoutai2" "$txtEiyouJoutaibasho2" "$rdEijouJoutai3" "$txtEiyouJoutaibasho3" "$rdSuibunSessyu1" "$slSuibunSessyuSeigen1" \
...
ycat ${tmp}-assessment_kihonjoho ${tmp}-asessment_kadai ${tmp}-assessment_ADL ${tmp}-assessment_IADL ${tmp}-sonota > ${tmp}-lv1data_assessment_kaigo
...
while read kazokunum ; do
   ...
   echo "$Riyoushaid" "$hdShokaiSakuseidate" "$nyukyoshashimei" "$nyukyoshazokugara" "$nyukyoshanenrei" "$nyukyoshakenkou" >> ${tmp}-lv1data_assessment_kazoku
done
...
while read kinkyunum ; do
   ...
   echo "$Riyoushaid" "$hdShokaiSakuseidate" "$kinkyushimei" "$kinkyuzokugara" "$kinkyujusho" "$kinkyudenwa" >> ${tmp}-lv1data_assessment_kinkyu
done
...
while read servicenum ; do
   ...
   echo "$Riyoushaid" "$hdShokaiSakuseidate" "$serviceshubetsu" "$servicejigyousho" "$servicebiko" >> ${tmp}-lv1data_assessment_service
done
...
while read jushinnum ; do
   ...
   echo "$Riyoushaid" "$hdShokaiSakuseidate" "$byoumei" "$isha" "$shinryoukamoku" "$denwa" "$joukyou" >> ${tmp}-lv1data_assessment_byouin
done
...
while read syoubyounum ; do
   ...
   echo "$Riyoushaid" "$hdShokaiSakuseidate" "$syoubyoumei" "$syoubyouumu" "$syoubyoubui" >> ${tmp}-lv1data_assessment_shoubyou
done

cp -p ${tmp}-lv1data_assessment_shoubyou assessment_shoubyou




# アセスメント本体LV1データ設置
lv1data_dir=${input_dir}/ASSESSMENT_KAIGO/${Riyoushaid}
mkdir -p ${lv1data_dir}

cp ${tmp}-lv1data_assessment_kaigo ${lv1data_dir}/ASSESSMENT_KAIGO.${sdaytime}.$$

if [ ! -s ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAIGO ]; then
    mkdir -p ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}
    touch ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAIGO
fi

# データ作成
cat ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAIGO |
LANG=C sort -k1,2                                                > ${tmp}-base_assessment_kaigo
upl key=1@2 ${tmp}-base_assessment_kaigo ${tmp}-lv1data_assessment_kaigo > ${tmp}-lv4_assessment_kaigo

cp -p ${tmp}-lv4_assessment_kaigo ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAIGO


# アセスメント家族構成データLV1設置
cp ${tmp}-lv1data_assessment_kazoku ${lv1data_dir}/ASSESSMENT_KAZOKU_KAIGO.${sdaytime}.$$

if [ ! -s ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAZOKU_KAIGO ]; then
    touch ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAZOKU_KAIGO
fi

# データ作成
cat ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAZOKU_KAIGO |
LANG=C sort -k1,2                                                       |
# 同利用者、同一作成日時のレコードを一旦消す。
gawk '$1!="'$Riyoushaid'"&&$2!="'$hdShokaiSakuseidate'"'                > ${tmp}-base_assessment_kazoku_kaigo

cat ${tmp}-base_assessment_kazoku_kaigo ${tmp}-lv1data_assessment_kazoku     |
LANG=C sort -k1,2                          > ${tmp}-lv4_assessment_kazoku_kaigo
cp -p ${tmp}-lv4_assessment_kazoku_kaigo ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KAZOKU_KAIGO


# アセスメント緊急連絡先データLV1設置
cp ${tmp}-lv1data_assessment_kinkyu ${lv1data_dir}/ASSESSMENT_KINKYU_KAIGO.${sdaytime}.$$

if [ ! -s ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KINKYU_KAIGO ]; then
    touch ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KINKYU_KAIGO
fi

# データ作成
cat ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KINKYU_KAIGO |
LANG=C sort -k1,2                                                       |
# 同利用者、同一作成日時のレコードを一旦消す。
gawk '$1!="'$Riyoushaid'"&&$2!="'$hdShokaiSakuseidate'"'                > ${tmp}-base_assessment_kinkyu_kaigo

cat ${tmp}-base_assessment_kinkyu_kaigo ${tmp}-lv1data_assessment_kinkyu     |
LANG=C sort -k1,2                          > ${tmp}-lv4_assessment_kinkyu_kaigo
cp -p ${tmp}-lv4_assessment_kinkyu_kaigo ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_KINKYU_KAIGO


# アセスメントサービス利用データLV1設置
cp ${tmp}-lv1data_assessment_service ${lv1data_dir}/ASSESSMENT_SERVICE_KAIGO.${sdaytime}.$$

if [ ! -s ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SERVICE_KAIGO ]; then
    touch ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SERVICE_KAIGO
fi

# データ作成
cat ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SERVICE_KAIGO |
LANG=C sort -k1,2                                                       |
# 同利用者、同一作成日時のレコードを一旦消す。
gawk '$1!="'$Riyoushaid'"&&$2!="'$hdShokaiSakuseidate'"'                > ${tmp}-base_assessment_service_kaigo

cat ${tmp}-base_assessment_service_kaigo ${tmp}-lv1data_assessment_service |
LANG=C sort -k1,2                          > ${tmp}-lv4_assessment_service_kaigo
cp -p ${tmp}-lv4_assessment_service_kaigo ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SERVICE_KAIGO


# アセスメント病院利用データLV1設置
cp ${tmp}-lv1data_assessment_byouin ${lv1data_dir}/ASSESSMENT_BYOUIN_KAIGO.${sdaytime}.$$

if [ ! -s ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_BYOUIN_KAIGO ]; then
    touch ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_BYOUIN_KAIGO
fi

# データ作成
cat ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_BYOUIN_KAIGO |
LANG=C sort -k1,2                                                       |
# 同利用者、同一作成日時のレコードを一旦消す。
gawk '$1!="'$Riyoushaid'"&&$2!="'$hdShokaiSakuseidate'"'                > ${tmp}-base_assessment_byouin_kaigo

cat ${tmp}-base_assessment_byouin_kaigo ${tmp}-lv1data_assessment_byouin   |
LANG=C sort -k1,2                          > ${tmp}-lv4_assessment_byouin_kaigo
cp -p ${tmp}-lv4_assessment_byouin_kaigo ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_BYOUIN_KAIGO


# アセスメント病院利用データLV1設置
cp ${tmp}-lv1data_assessment_shoubyou ${lv1data_dir}/ASSESSMENT_SHOUBYOU_KAIGO.${sdaytime}.$$

if [ ! -s ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SHOUBYOU_KAIGO ]; then
    touch ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SHOUBYOU_KAIGO
fi

# データ作成
cat ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SHOUBYOU_KAIGO |
LANG=C sort -k1,2                                                       |
# 同利用者、同一作成日時のレコードを一旦消す。
gawk '$1!="'$Riyoushaid'"&&$2!="'$hdShokaiSakuseidate'"'                > ${tmp}-base_assessment_shoubyou_kaigo

cat ${tmp}-base_assessment_shoubyou_kaigo ${tmp}-lv1data_assessment_shoubyou   |
LANG=C sort -k1,2                          > ${tmp}-lv4_assessment_shoubyou_kaigo
cp -p ${tmp}-lv4_assessment_shoubyou_kaigo ${pompa_dir}/ASSESSMENT_KAIGO/${Riyoushaid}/ASSESSMENT_SHOUBYOU_KAIGO
