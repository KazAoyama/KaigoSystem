#!/bin/bash
#
# GEKKAN_RIYOU_KEIKAKU_INSATSU_YOBO.CGI : ケアプラン（介護）MENUCGI
#
# Usage : GEKKAN_RIYOU_KEIKAKU_INSATSU_YOBO.CGI
#
...
case "${MODE}" in
  #初期表示
  init )
    ${cgi_dir}/GEKKAN_RIYOU_KEIKAKU_INSATSU_YOBO.DISPLAY ${tmp}-name > ${tmp}-html
    ;;

  #印刷する場合
  allprint )
    mkdir -p ${home_dir}/CAREPLAN/DOWNLOAD/IKKATSU_GEKKAN_YOBO/$USER_ID
    rm -f ${home_dir}/CAREPLAN/DOWNLOAD/IKKATSU_GEKKAN_YOBO/$USER_ID/*
    # 複数の利用者リストの対応
    ...
    if [ ${Riyouhyou} = "1" ] ; then
        ${cgi_dir}/RIYOUHYOU_YOBO.SEARCH ${tmp}-name > ${tmp}-search
        ${cgi_dir}/RIYOUHYOU_YOBO.PRINT $tmp-name ${tmp}-search >> ${tmp}-html            
    fi
    if [ ${RiyouhyouBeppyou} = "1" ] ; then
        ${cgi_dir}/RIYOUHYOU_BEPPYOU_YOBO_PRINT.SEARCH ${tmp}-name > ${tmp}-search
        ${cgi_dir}/RIYOUHYOU_BEPPYOU_YOBO.PRINT $tmp-name ${tmp}-search >> ${tmp}-html            
    fi
...
# 提供票、提供票別表は関係ある事業者ごとに出力する。
    if [ ${Teikyouhyou} = "1" ] ; then
      ...
      if [ -s ${pompa_dir}/GETSUGAKU_RIYOU_YOBO/${Riyoushaid}/SERVICE_YOTEI_YOBO ] ; then
         riyouhyou_timestamp=$(ls --full-time ${pompa_dir}/GETSUGAKU_RIYOU_YOBO/${Riyoushaid}/SERVICE_YOTEI_YOBO | awk '{gsub(/-/,"",$6);gsub(/:/,"",$7);print $6substr($7,1,6)}')
      fi
      # 利用票別表ファイルの取得
...
#
       ...
       while read taishoujigyousho ; do
         ...
         ${cgi_dir}/TEIKYOUHYOU_YOBO.SEARCH ${tmp}-name > ${tmp}-search
         ${cgi_dir}/TEIKYOUHYOU_YOBO.PRINT $tmp-name ${tmp}-search >> ${tmp}-html
       : ;done
    fi
...
#
       ...
       while read taishoujigyousho ; do
         echo "Jigyoushaid" $taishoujigyousho >> ${tmp}-name
         ${cgi_dir}/TEIKYOUHYOU_BEPPYOU_YOBO_PRINT.SEARCH ${tmp}-name > ${tmp}-search
         ${cgi_dir}/TEIKYOUHYOU_BEPPYOU_YOBO.PRINT $tmp-name ${tmp}-search >> ${tmp}-html
       : ;done
    fi

    gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sPAPERSIZE=a4 -dNOPLATFONTS -sOutputFile=${home_dir}/CAREPLAN/DOWNLOAD/IKKATSU_GEKKAN_YOBO/$USER_ID/${USERID}_GEKKAN_RIYOU_KEIKAKU.PDF ${home_dir}/CAREPLAN/DOWNLOAD/IKKATSU_GEKKAN_YOBO/$USER_ID/*.pdf
# HTML の出力
cat << EOF > ${tmp}-html
<HTML>
  <HEAD>
    <SCRIPT type="text/javascript" language="Javascript">
      ...
      window.onload=init
      function init(){
           window.open("/E-LIFE/CAREPLAN/DOWNLOAD/IKKATSU_GEKKAN_YOBO/${USER_ID}/${USERID}_GEKKAN_RIYOU_KEIKAKU.PDF","pdfprint");
           window.self.close();
      }
      //]]>
    </SCRIPT>
